...
from script.ui.bosses.hud_matita_pereira import BossHudMatitaPereira
from script.data.dialogs.level_1_2.matita_pereira.dialog_mp_1_2 import Dialogo_MP_1_2
from script.data.dialogs.level_1_2.matita_pereira.dialog_mp_1_3 import Dialogo_MP_1_3


class Boss_Matita_Pereira(Obj):
    """
    Boss Matita Pereira.
    ...
    INTRO_DIALOGUE_CLASS = Dialogo_MP_1_2
    FINAL_DIALOGUE_CLASS = Dialogo_MP_1_3
    CORPSE_OFFSET_Y = 0
    CHATBOX_DIR = "assets/chatChar/level_1_2/Matita_Pereira"
    
    DEFAULT_SIZE = (275, 300)
    DEFAULT_POSITION = (900, 190)
    
    DEATH_PORTRAIT = os.path.join(
        "assets", "charsSprite", "bosses", "Matita_Pereira", "MP_D.png"
    )
    
    def __init__(self, position, groups, size=None, debug_hitbox=False):
        if size is None:
        ...
        image_path = os.path.join("assets", "charsSprite", "bosses", "Matita_Pereira", "MP_I.png")
        ...
        # ---------------------------------------------------------
        # ATAQUE AÉREO DA MATITA PEREIRA
        # ---------------------------------------------------------
        # A ação "run_attack" foi adaptada para duas etapas:
        # 1) fly_up: sobe em diagonal usando MP_JF.png;
        # 2) dive: mergulha em direção ao ponto memorizado do Player usando MP_FD.png.
        self.air_attack_phase = None
        self.air_target_x = None
        self.air_target_y = None
        self.air_target_bottom = None
        self.air_peak_y = None

        self.fly_up_speed_x = 5
        self.fly_up_speed_y = 7
        self.dive_speed_x = 12
        self.dive_speed_y = 10
        self.air_height = 130

        # Linha real do chão da fase.
        # O ataque aéreo deve terminar usando o bottom da sprite nesta linha,
        # para evitar que a Matita pare abaixo do nível do chão.
        self.ground_y = 485
        self.air_target_bottom = None
    ...
    def _load_animations(self):
        """
        Carrega as animações da Matita Pereira.

        Os arquivos precisam estar em:
            assets/charsSprite/bosses/Matita_Pereira
        """
        # Idle/parado.
        idle_files = [
            "MP_I.png"
        ...
        # Ataque aéreo.
        # MP_JF = início do voo/subida diagonal.
        # MP_FD = mergulho/ataque aéreo.
        fly_up_files = [
            "MP_JF.png",
        ]
        ...
        self.animations["fly_up"] = self._load_frames(fly_up_files)
        self.animations["dive"] = self._load_frames(dive_files)
    ...
    def _load_frames(self, filenames):
        ...
        for filename in filenames:
            path = os.path.join("assets", "charsSprite", "bosses", "Matita_Pereira", filename)
    ...
    def _run_attack_to_memory_point(self, now):
        """
        Executa o ataque aéreo da Matita Pereira.

        Fase 1 - fly_up:
        - usa a sprite MP_JF;
        - sobe em diagonal na direção do Player memorizado.

        Fase 2 - dive:
        - usa a sprite MP_FD;
        - mergulha na direção do ponto memorizado do Player;
        - ao chegar ao destino, volta para idle e aguarda novo ciclo.

        Importante:
        A posição do Player NÃO é recalculada durante o ataque.
        A Matita ataca o ponto em que o Player estava no momento do sorteio.
        """

        # ---------------------------------------------------------
        # 1 - INÍCIO DO ATAQUE AÉREO
        # ---------------------------------------------------------
        if self.air_attack_phase is None:
            self.air_attack_phase = "fly_up"

            self.air_target_x = self.run_target_x

            # Mantém o destino vertical exatamente na linha do chão.
            # Usa o bottom da sprite, e não o centery.
            self.air_target_bottom = self.ground_y

            # Altura máxima da subida.
            self.air_peak_y = self.rect.centery - self.air_height

            # Define para qual lado ela deve olhar durante a subida.
            self.facing_right = self.air_target_x > self.rect.centerx

        # ---------------------------------------------------------
        # 2 - FASE DE SUBIDA DIAGONAL
        # ---------------------------------------------------------
        if self.air_attack_phase == "fly_up":
            self.state = "fly_up"

            if self.facing_right:
                self.rect.x += self.fly_up_speed_x
            else:
                self.rect.x -= self.fly_up_speed_x

            self.rect.y -= self.fly_up_speed_y

            if self.rect.centery <= self.air_peak_y:
                self.air_attack_phase = "dive"

            self._rebuild_hitboxes()
            return

        # ---------------------------------------------------------
        # 3 - FASE DE MERGULHO / ATAQUE AÉREO
        # ---------------------------------------------------------
        if self.air_attack_phase == "dive":
            self.state = "dive"

            dx = self.air_target_x - self.rect.centerx
            dy = self.air_target_bottom - self.rect.bottom

            if abs(dx) <= self.run_arrival_distance and abs(dy) <= self.run_arrival_distance:
                self.rect.centerx = self.air_target_x
                self.rect.bottom = self.air_target_bottom
            ...
            if dx > 0:
                self.rect.x += min(self.dive_speed_x, abs(dx))
                self.facing_right = True
            elif dx < 0:
                self.rect.x -= min(self.dive_speed_x, abs(dx))
                self.facing_right = False
            ...
            if dy > 0:
                self.rect.y += min(self.dive_speed_y, abs(dy))
            elif dy < 0:
                self.rect.y -= min(self.dive_speed_y, abs(dy))
    ...
    def get_intro_dialogue_class(cls):
        """
        Retorna a classe de diálogo inicial específica deste boss.
        """
    ...
    def get_final_dialogue_class(cls):
        """
        Retorna a classe de diálogo final específica deste boss.
        """
    ...
    def interact(self, player):
        ...
        if self.rect.colliderect(player.rect):
            print("Matita Pereira: MEU ASSOVIO ECOA PELA MATA! PREPARE-SE PARA LUTAR.")
